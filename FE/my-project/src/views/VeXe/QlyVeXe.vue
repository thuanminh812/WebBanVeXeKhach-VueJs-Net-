<template>
    ...
    <section id="content">
        ...
        <h2 style="margin-left:150px">Quản lý vé xe</h2>
		<button class="btn btn-info mb-3" style="margin-left:150px"><a href="formvexe" class="text-white">Thêm mới</a></button>
    ...
    <table class="table">
        ...
        <tbody>
            <tr v-for="thongtinve in thongtinves" :key="thongtinve.maVe">
                <td>{{thongtinve.tenKh}}</td>
                <td>{{thongtinve.soDt}}</td>
                <td>{{thongtinve.tuyen}}</td>
                <td>{{thongtinve.thoiGian}}</td>
                <td>{{thongtinve.loaiVe}}</td>
				<router-link :to="{ name: 'formedit', params: {name: thongtinve.maVe}}">
				<button class="btn btn-success">Cập nhật</button> |
			</router-link>
				<button class="btn btn-danger" @click="onDelete(thongtinve.maVe)">Xóa</button>
            </tr>
        </tbody>
    </table>
  </div>
...
export default {
 ...
 created(){
  ...
  if (result.isConfirmed) {
	axios.delete(`http://localhost:5211/api/ThongTinVe/${name}`) .then(response => {
		console.log(response.data)
		if(response.data){
