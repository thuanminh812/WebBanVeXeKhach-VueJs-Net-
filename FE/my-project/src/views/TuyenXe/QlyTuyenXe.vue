<template>
    ...
    <section id="content">
        ...
        <h2 style="margin-left:150px">Quản lý tuyến xe</h2>
		<button class="btn btn-info mb-3" style="margin-left:150px"><a href="formtuyenxe" class="text-white">Thêm mới</a></button>
...
<tbody>
    <tr v-for="thongtintuyen in thongtintuyens" v-bind:key="thongtintuyen.tuyen">
        <td>{{thongtintuyen.tuyen}}</td>
        <td>{{thongtintuyen.maTuyen}}</td>
				<router-link :to="{ name: 'formedittuyen', params: {name: thongtintuyen.tuyen}}">
				<button class="btn btn-success">Cập nhật</button> |
			</router-link>
				<button class="btn btn-danger" @click="onDelete(thongtintuyen.tuyen)">Xóa</button>
            </tr>
        </tbody>
    </table>
  </div>
...
export default {
 ...
 created(){
  ...
  if (result.isConfirmed) {
	axios.delete(`http://localhost:5211/api/ThongTinTuyen/${name}`) .then(response => {
		console.log(response.data)
		if(response.data){
