<template>
    ...
    <section id="content">
        ...
        <h2 style="margin-left:150px">Quản lý khách hàng</h2>
		<button class="btn btn-info mb-3" style="margin-left:150px"><a href="formkhachhang" class="text-white">Thêm mới</a></button>
...
<tbody>
    <tr v-for="khachhang in khachhangs" v-bind:key="khachhang.hoTen">
        <td>{{khachhang.hoTen}}</td>
        <td>{{khachhang.diaChi}}</td>
				<router-link :to="{ name: 'formeditkhachhang', params: {name: khachhang.hoTen}}">
				<button class="btn btn-success">Cập nhật</button> |
			</router-link>
				<button class="btn btn-danger" @click="onDelete(khachhang.hoTen)">Xóa</button>
            </tr>
        </tbody>
    </table>
  </div>
...
export default {
 ...
 created(){
        axios.get('http://localhost:5211/api/KhachHang')
        .then(response=>{
  ...
  if (result.isConfirmed) {
	axios.delete(`http://localhost:5211/api/KhachHang/${name}`) .then(response => {
		console.log(response.data)
		if(response.data){
