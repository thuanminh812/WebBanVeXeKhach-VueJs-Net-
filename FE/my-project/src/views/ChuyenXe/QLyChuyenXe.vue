<template>
    ...
    <section id="content">
        ...
        <h2 style="margin-left:150px">Quản lý Chuyến xe</h2>
		<button class="btn btn-info mb-3" style="margin-left:150px"><a href="formchuyenxe" class="text-white">Thêm mới</a></button>
    ...
    <table class="table">
        ...
        <tbody>
            <tr v-for="thongtinchuyen in thongtinchuyens" v-bind:key="thongtinchuyen.maChuyen">
                <td>{{thongtinchuyen.maChuyen}}</td>
                <td>{{thongtinchuyen.tuyen}}</td>
                <td>{{thongtinchuyen.trangThai}}</td>
				<router-link :to="{ name: 'formeditchuyen', params: {name: thongtinchuyen.maChuyen}}">
				<button class="btn btn-success">Cập nhật</button> |
			</router-link>
				<button class="btn btn-danger" @click="onDelete(thongtinchuyen.maChuyen)">Xóa</button>
            </tr>
        </tbody>
    </table>
  </div>
...
export default {
 ...
 created(){
  ...
  if (result.isConfirmed) {
	axios.delete(`http://localhost:5211/api/ThongTinChuyen/${name}`) .then(response => {
		console.log(response.data)
		if(response.data){
